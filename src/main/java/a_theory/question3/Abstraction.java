package a_theory.question3;

public class Abstraction {

    //todo
    // Abstra-ca-dabra

    //todo p1
    // In your words (do not use wiki definitions)
    // What is abstraction? (P.S It has nothing to do with keyword abstract)
    // Answer: Abstraction is the approach which involves separating the object characteristics and using only the ones
    // that are required for a particular operations, neglecting the others. More practically it means that only
    // essential properties of the object are accessible by the user

    //todo p2
    // Create an abstraction of an animal of your choice.
    // It should have at least 1 property and 1 method.
    // Create it as a real java class inside this package.
}
